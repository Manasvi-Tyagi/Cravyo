...
const CommentsModal = ({ isOpen, foodId, onClose, reelData, onCommentAdded }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://localhost:1234/api/food/comments/${foodId}`, { withCredentials: true })
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const response = await axios.get('http://localhost:1234/api/auth/user/me', { withCredentials: true })
  ...
  const handlePostComment = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:1234/api/food/comments', { foodId, text: inputValue }, { withCredentials: true })
      const newComment = {
        _id: response.data.comment._id,
        user: { name: response.data.comment.user.name, avatar: response.data.comment.user.profileImage || '👤' },
      ...
      setInputValue('')
      if (onCommentAdded) onCommentAdded(foodId)
    } catch (error) {
  ...
  const toggleLikeComment = async (commentId) => {
    ...
    try {
      const response = await axios.post('http://localhost:1234/api/food/comments/like', 
        { commentId }, 
  ...
  const saveEditedComment = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:1234/api/food/comment/${editingCommentId}`, 
        { text: editInputValue }, 
        { withCredentials: true }
      )
  ...
  const deleteComment = async (commentId) => {
    ...
    try {
      await axios.delete(`http://localhost:1234/api/food/comment/${commentId}`, 
        { withCredentials: true }
      )
