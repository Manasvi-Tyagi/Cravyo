...
const Saved = () => {
  ...
  React.useEffect(() => {
    axios
      .get('http://localhost:1234/api/auth/food-partner/me', { withCredentials: true })
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      axios.get('http://localhost:1234/api/food/', { withCredentials: true }),
      axios.get('http://localhost:1234/api/food/liked', { withCredentials: true }),
      axios.get('http://localhost:1234/api/food/saved', { withCredentials: true })
    ])
      .then(([foodsRes, likedRes, savedRes]) => {
        setVideos(foodsRes.data.food || foodsRes.data)
        setLikedIds(new Set((likedRes.data.foods || []).map(f => f._id)))
        setSavedIds(new Set((savedRes.data.foods || []).map(f => f._id)))
      })
  ...
  const toggleLike = async (foodId) => {
    ...
    try {
      await axios.post('http://localhost:1234/api/food/like', { foodId }, { withCredentials: true })
    } catch (e) {
  ...
  const toggleSave = async (foodId) => {
    ...
    try {
      await axios.post(
        'http://localhost:1234/api/food/save',
        { foodId },
