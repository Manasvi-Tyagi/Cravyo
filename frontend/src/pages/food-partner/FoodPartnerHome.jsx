...
export default function FoodPartnerHome() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const foodsRes = await axios.get('http://localhost:1234/api/food/', { withCredentials: true })
  ...
  const toggleSave = async (foodId) => {
    ...
    try {
      await axios.post('http://localhost:1234/api/food/save', { foodId }, { withCredentials: true })
      if (!alreadySaved) {
        navigate('/saved')
      }
    } catch (e) {
